...
from hummingbot.funding_arbitrage.fixed_market_specs import PVPriceType, get_market_pair_info
...
class MarketsRecorder:
    ...
    def __init__(
        ...
        for market in self._markets:
            ...
            market.add_trade_fills_from_market_recorder(
                {TradeFillOrderDetails(tf.market, tf.exchange_trade_id, tf.symbol) for tf in trade_fills}
            )
        ...
        self._position_metrics = PositionMetrics(self.sql_manager)
    ...
    def get_orders_for_config_and_market(
        self,
        config_file_path: str,
        ...
        with self._sql_manager.get_new_session() as session:
            ...
            query: Query = session.query(Order).filter(*filters).order_by(Order.creation_timestamp)
    ...
    def _did_create_order(
        ...
        with self._sql_manager.get_new_session() as session:
            with session.begin():
                ...
                order_status: OrderStatus = OrderStatus(order=order_record, timestamp=timestamp, status=event_type.name)
    ...
    def _did_fill_order(self, event_tag: int, market: ConnectorBase, evt: OrderFilledEvent):
        if threading.current_thread() != threading.main_thread():
        ...
        with self._sql_manager.get_new_session() as session:
            with session.begin():
                ...
                order_status: OrderStatus = OrderStatus(order_id=order_id, timestamp=timestamp, status=event_type.name)
                try:
    ...
    def _did_complete_funding_payment(self, event_tag: int, market: ConnectorBase, evt: FundingPaymentCompletedEvent):
        if threading.current_thread() != threading.main_thread():
        ...
        with self._sql_manager.get_new_session() as session:
            with session.begin():
                ...
                if payment_record is None:
                    # Find the funding trade this payment belongs to, if any
                    associated_trade = FundingTrade.find_funding_trade(
                        session, timestamp, market.display_name, evt.trading_pair
                    )

                    base, quote = evt.trading_pair.split("-")
                    market_pair_info = get_market_pair_info(market.display_name, base, quote)

                    print(associated_trade)
                    trade_id = associated_trade.id if associated_trade else None
                    price_type = market_pair_info.price_type if market_pair_info else PVPriceType.UNKNOWN
                    trade_position_value = None
                    trade_position_exposure = None

                    if associated_trade is None:
                        # Not sure how this would happen. If it does, its a potential bug
                        self.logger().error(
                            f"Could not find trade associated with funding payment for {market.display_name}:{evt.trading_pair} at timestamp {timestamp}"
                        )
                    else:
                        trade_position_size = self._position_metrics.get_position_size(
                            associated_trade.start_time,
                            associated_trade.end_time,
                            market.display_name,
                            evt.trading_pair,
                        )

                        # Since market and evt would have to have matched with either side m-p of the trade, this works
                        position_side = (
                            "long"
                            if associated_trade.long_market == market.display_name
                            and associated_trade.long_pair == evt.trading_pair
                            else "short"
                        )

                        avg_entry_price = self._position_metrics.get_position_avg_entry_price(
                            associated_trade.start_time,
                            associated_trade.end_time,
                            market.display_name,
                            evt.trading_pair,
                        ...
                        if price_type == PVPriceType.AVG_ENTRY:
                            # Since in this case the exchange calculates position value using average entry price,
                            # it is the same as the position "exposure" or initial position value
                            trade_position_value = trade_position_size * avg_entry_price
                            trade_position_exposure = trade_position_size * avg_entry_price
                        else:
                            # In these cases we assume no overlapping directional bets or other trades, so we can
                            # determine position amount directly from funding payment info
                            trade_position_value = evt.amount / evt.funding_rate
                            trade_position_exposure = trade_position_size * avg_entry_price

                    funding_payment_record: FundingPayment = FundingPayment(
                        timestamp=timestamp,
                        config_file_path=self.config_file_path,
                        market=market.display_name,
                        ...
                        trade_position_value=trade_position_value,
                        trade_position_exposure=trade_position_exposure,
                    )
    ...
    def append_to_csv(self, trade: TradeFill):
        ...
        age = (
            pd.Timestamp(int((trade.timestamp * 1e-3) - (trade.order.creation_timestamp * 1e-3)), unit="s").strftime(
                "%H:%M:%S"
            )
            if (trade.order is not None and "//" not in trade.order_id)
            else "n/a"
        )
        field_names += ("age",)
        field_data += (age,)
            ...
            with session.begin():
                ...
                if order_record is not None:
                    ...
                    order_status: OrderStatus = OrderStatus(
                        order_id=order_id, timestamp=timestamp, status=event_type.name
                    )
                    session.add(order_status)
                    self.save_market_states(self._config_file_path, market, session=session)

    def _did_cancel_order(self, event_tag: int, market: ConnectorBase, evt: OrderCancelledEvent):
        self._update_order_status(event_tag, market, evt)

    def _did_fail_order(self, event_tag: int, market: ConnectorBase, evt: MarketOrderFailureEvent):
        self._update_order_status(event_tag, market, evt)
    ...
    def _did_expire_order(self, event_tag: int, market: ConnectorBase, evt: OrderExpiredEvent):
        self._update_order_status(event_tag, market, evt)

    def _did_update_range_position(
        self,
        event_tag: int,
        connector: ConnectorBase,
        evt: Union[
            RangePositionLiquidityAddedEvent, RangePositionLiquidityRemovedEvent, RangePositionFeeCollectedEvent
        ],
    ):
        if threading.current_thread() != threading.main_thread():
    ...
    def _did_close_position(self, event_tag: int, connector: ConnectorBase, evt: RangePositionClosedEvent):
        if threading.current_thread() != threading.main_thread():
        ...
        with self._sql_manager.get_new_session() as session:
            with session.begin():
                rp_fees: RangePositionCollectedFees = RangePositionCollectedFees(
                    config_file_path=self._config_file_path,
                    strategy=self._strategy_name,
                    token_id=evt.token_id,
                    token_0=evt.token_0,
